{- HLINT ignore -}
{-# OPTIONS_GHC -w #-}
module HelVM.HelPS.HS2Lazy.Parser where
...
import           Prelude                            hiding (Alt, Ap, Const, Type, guard, lift, lookupEnv, modify, newTVar)
...
aexp = do { i <- qvar; return (Var i) }
   ...
   <|> do es <- parenList expr
          if length es == 1 then return (List.head es) else return (Tuple es)
...
apat = do v <- var
   ...
   <|> do ps <- parenList pat
          if length ps == 1 then return (List.head ps) else return (PTuple ps)
...
parse input = case runParser program () "stdin" input of
                Left err  -> error $ toText ("parse error at " ++ show err)
                Right tds -> tds

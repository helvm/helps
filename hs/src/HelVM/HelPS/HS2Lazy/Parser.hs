...
import           Prelude                            hiding (Alt, Ap, Const, Type, guard, many, (<|>))
...
aexp =
  do i <- qvar; return (Var i)
    ...
    <|> do
      es <- parenList expr
      if length es == 1 then return (unsafeHead es) else return (Tuple es)
    <|> do
      es <- bracketList expr
...
apat =
  do
    ...
    <|> do
      ps <- parenList pat
      if length ps == 1 then return (unsafeHead ps) else return (PTuple ps)
    <|> do
      ps <- bracketList pat

    ...
    case lookup p' (reParam re) of
      ...
      Nothing ->
          case byInst (reClass re) p' of
            ...
            Nothing       -> fromMaybe e (resolveSuper re pes p')
...
tiImpls ce env bs =
    do ts <- mapM (\_ -> newTVar Star) bs
       let is    = map fst bs
           ...
           altss = map snd bs
       pssass <- zipWithM (tiAlts ce env') altss ts
       s      <- getSubst
       let ps'     = apply s (concatMap fst pssass)
           ts'     = apply s ts
           fs      = tv (apply s env)
           vss     = map tv ts'
...
tiBindGroup ce env (es,iss) =
  do let as = [ v:>:sc | (v,sc,alts) <- es ]
     ...
     return (ps ++ concatMap fst qses_s, as' ++ as, (map snd qses_s, iss'))

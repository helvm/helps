...
data Tyvar = Tyvar Id Kind deriving stock (Eq)

instance Show Tyvar where
  show (Tyvar id_ _) = id_
...
data Synonym = Synonym Id Kind [Tyvar] Type deriving stock (Eq)

instance Show Synonym where
  show (Synonym id_ _ _ _) = id_
...
preludeConstrs =
  ...
  where
    a_ = TVar (Tyvar "a" Star)
    constrs =
      [ ("True", 0, 1, tBool, tBool),
        ("False", 0, 2, tBool, tBool),
        (":", 2, 1, tList, a_ `fn` list a_ `fn` list a_),
        ("[]", 0, 2, tList, list a_)
      ]
...
eTrue = Con con
  where
    con = fromJust $ find (\c -> conName c == "True") preludeConstrs

eFalse :: Expr
eFalse = Con con
  where
    con = fromJust $ find (\c -> conName c == "False") preludeConstrs

eCons :: Expr
eCons = Con con
  where
    con = fromJust $ find (\c -> conName c == ":") preludeConstrs
...
instance Show Pred where
  showsPrec _ (IsIn id_ t) = (id_ ++) . (' ' :) . shows t
...
findAssump id_ []                = fail ("unbound identifier: " ++ id_)
findAssump id_ ((i :>: sc) : as) = if i == id_ then return sc else findAssump id_ as
...
tupleSelector id_ k n = (id_, [([pat], Rhs expr)])
  where

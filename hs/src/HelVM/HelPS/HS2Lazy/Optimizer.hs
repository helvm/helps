...
optExpr (Case _ _)                = error "Case"
optExpr (RecPH _)                 = error "RecPH"
optExpr (ClassPH _)               = error "ClassPH"
optExpr (Lambda (_, (Where _ _))) = error "Lambda Where"
optExpr (Lambda (_, (Guarded _))) = error "Lambda Guarded"

optBindGroup :: [Impl] -> BindGroup
optBindGroup is = ([], [is']) where
...
substVar e' v (Case e gds) = Case (substVar e' v e) [(p, if v `elem` patVars p then rhs else subVarRhs e' v rhs) | (p, rhs) <- gds]

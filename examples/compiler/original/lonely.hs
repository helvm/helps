#ifdef EMBEDDED
{- HLINT ignore -}
module HelVM.HelPS.Compiler.Compiler.Lonely where
import Prelude (Bool(..), Char, Int, String, exitSuccess, succ)
...
#endif
#ifdef EMBEDDED
unify a b = maybe Nothing \s -> maybeFMap (@@ s) (mgu unify (apply s a) (apply s b));
#endif
...
inferDefs ienv defs typed = flst defs (Right $ reverse typed) \edef rest -> case edef of
  { Left def -> fpair def \s expr ->
    fpair (infer typed [(s, TV "self!")] expr (Just [], 0)) \ta msn ->
      fpair msn \ms _ -> case maybeFMap (prove ienv s ta) (unify (TV "self!") (fst ta) ms) of
    { Nothing -> Left ("bad type: " ++ s)
    ; Just qa -> inferDefs ienv rest ((s, qa):typed)
    }
  ; Right inst -> inferDefs ienv rest (inferInst ienv typed inst:typed)
...
#endif
#ifdef EMBEDDED
getContents = getChar >>= \n -> ife (n <= 255) (ioFMap (chr n:) getContents) (pure []);
#endif
